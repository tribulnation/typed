...
_CHANNEL_PARAM = re.compile(r'\{(\w+)\}')


def _resolve_channel(
  channel: str, params: SubscriptionParams,
) -> 'tuple[str, dict[str, Any]]':
  """Interpolate a `{name}` placeholder in a channel template (`candle{interval}`,
  `books{depth}`) from `params`, once, before it becomes `request_channel` --
  `typed_core.ws.StreamsRpc._subscribe_impl` registers `message_keys` under
  `request_channel` verbatim and later looks incoming pushes' own echoed channel name up
  in that same dict (`on_msg`), so `request_channel` and the real wire channel a push
  actually carries (`channel_of`) have to be the identical *resolved* string -- passing
  the raw template through as `request_channel` (this method's own previous shape)
  registers `"candle{interval}"` while every real push arrives tagged `"candle1m"`, a
  permanent miss that silently drops every notification into `on_msg`'s `if (q :=
  self.subscriptions.get(channel)) is not None` no-op branch and leaves
  `anext(stream.__aiter__())` awaiting a queue that will never receive anything.

  The interpolated field is also dropped from what's spread into `build_arg`: the real
  venue bakes it into the channel name alone (`{"channel": "candle1m", ...}`, confirmed
  against every recorded example), never repeats it as a sibling field.
  """
  used: set[str] = set()
  ...
  resolved = _CHANNEL_PARAM.sub(substitute, channel)
  remaining = {key: value for key, value in params.items() if key not in used}
...
class BaseSocketStreamClient(StreamClient):
  ...
  def subscribe(
    ...
    *,
    ...
    resolved_channel, remaining_params = _resolve_channel(channel, params or {})
    local_key, message_key = self._local_key_and_message_key(resolved_channel, remaining_params)
    manager = self.public_conn.subscribe(
      local_key, remaining_params, request_channel=resolved_channel, message_key=message_key
    )
