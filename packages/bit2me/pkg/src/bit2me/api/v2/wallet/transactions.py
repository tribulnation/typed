...
class ListWalletTransactionsV2response(TypedDict):
  data: NotRequired[list[WalletTransaction]]
  """Transactions matching the query, newest first, up to `limit` entries per page."""
  total: NotRequired[int]
  """Total number of transactions matching the query, across all pages."""


validate_response = validator(ListWalletTransactionsV2response)
...
class Transactions(RpcEndpoint):
  async def transactions(
    ...
    *,
    ...
    References:
      - [Bit2Me API docs](https://api.bit2me.com/doc#tag/wallet/GET/v2/wallet/transaction)
    """
    params = {}
