...
class Transactions(RpcEndpoint):
  ...
  def transactions_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[WalletTransaction, int]:
    """Get user transactions.
    Transactions can be paginated using **offset** and **limit** parameters. For example, to get the third page and show 20 registers per page you would use the following query:

    ``` /v2/wallet/transaction?offset=40&limit=20 ```

    Paged variant of `transactions`: Advances `offset` by `len(rows)` and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(offset: int) -> tuple[Sequence[WalletTransaction], int | None]:
  ...
  async def transactions(
    ...
    *,
    ...
    return await self.request(
      request,
      method='GET',
      path='/v2/wallet/transaction',
      meta={},
      validate=validate,
      request_type=Request,
      response_type=ListWalletTransactionsV2Response,
    )
