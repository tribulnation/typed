...
class Transactions(RpcEndpoint):
  ...
  async def transactions_paged(
    ...
    *,
    ...
    Paged variant of `transactions`: Advances `offset` by `(limit if limit is not None else 20)` and stops once it has covered the `total` items the response reports, or after `max_pages` pages when one is given.
