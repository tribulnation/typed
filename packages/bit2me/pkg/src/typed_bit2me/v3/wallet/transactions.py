...
class Transactions(RpcEndpoint):
  ...
  async def transactions_paged(
    ...
    *,
    ...
    Paged variant of `transactions`: Passes each page's token back as `cursor` and stops when a response carries no `pageInfo.endCursor`, or after `max_pages` pages when one is given.
