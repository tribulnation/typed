...
class Request(TypedDict):
  ...
  """Opaque cursor for pagination. Pass the previous response's `pageInfo.endCursor` to fetch the next page; omit to fetch the first page."""
  limit: NotRequired[int]
  """Maximum number of entries to return."""
  year: NotRequired[int]
  """Filter movements to this specific year."""
  currency: NotRequired[str]
  """Currency the movements are denominated in."""
  ...
  tellerId: NotRequired[str]
  """Filter by the associated teller order id."""
...
class TransactionNetworkDetails(TypedDict):
  ...
  confirmedAt: NotRequired[TimestampIso | None]
  """When the transaction was confirmed on-chain (ISO 8601), or null while it has not yet been confirmed."""
...
class WalletTransaction(TypedDict):
  ...
  completedAt: NotRequired[TimestampIso | None]
  """When the transaction was completed (ISO 8601), or null while it has not completed."""
  canceledAt: NotRequired[TimestampIso | None]
...
class Transactions(RpcEndpoint):
  ...
  def transactions_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[WalletTransaction, str]:
    """Get user transactions with cursor-based pagination. Use the `endCursor` value from the previous response as the `cursor` parameter to fetch the next page.

    Paged variant of `transactions`: Passes each page's token back as `cursor` and stops when a response carries no `pageInfo.endCursor`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    References:
      - [Official docs](https://api.bit2me.com/doc#tag/Wallet/operation/getTransactionsV3)
    """

    async def next(cursor: str) -> tuple[Sequence[WalletTransaction], str | None]:
      ...
      state_0 = response.get('pageInfo')
      state = state_0.get('endCursor') if state_0 is not None else None
  ...
  async def transactions(
    ...
    *,
    ...
    if teller_id is not None:
      request['tellerId'] = teller_id
    return await self.request(
      request,
      method='GET',
      path='/v3/wallet/transaction',
      meta={},
      validate=validate,
      request_type=Request,
      response_type=ListWalletTransactionsV3Response,
    )
