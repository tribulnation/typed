...
When the pocket can't cover the amount, `preview` raises a `412` `ApiError` whose body still carries the fee Bit2Me would have charged. That's the only fee quote Bit2Me publishes; read it with `not_enough_funds`, see [Error Handling](../reference/error-handling.md#withdrawal-fees-from-a-failed-proforma).
