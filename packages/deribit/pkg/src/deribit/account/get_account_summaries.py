...
class AccountCurrencySummary(TypedDict):
  currency: str
...
class AccountSummaries(TypedDict):
  id: NotRequired[int]
  ...
  summaries: list[AccountCurrencySummary]
  """One margin summary per currency enabled on the account."""


validate_get_account_summaries = validator[AccountSummaries](AccountSummaries)


class GetAccountSummaries(RpcEndpoint):
  ...
  async def get_account_summaries(
    ...
    *,
    ...
    """Retrieves a per-currency list of account summaries for the authenticated user: balance, equity, available funds, and margin information for each currency. To retrieve summaries for a specific subaccount, use `subaccount_id`. When `extended` is `true`, additional account details are included alongside the per-currency `summaries`.
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/account-management/private-get_account_summaries)
    """
    params = {}
    ...
    return await self.authed_request(
      'private/get_account_summaries',
      params=params,
      validator=validate_get_account_summaries,
      validate=validate,
    )
