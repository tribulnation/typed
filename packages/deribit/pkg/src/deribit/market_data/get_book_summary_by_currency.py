...
validate_get_book_summary_by_currency = validator[list[BookSummaryItem]](
...
class GetBookSummaryByCurrency(RpcEndpoint):
  ...
  async def get_book_summary_by_currency(
    ...
    **Note:** For real-time updates, we recommend using the WebSocket subscription to `ticker.{instrument_name}.{interval}` instead of polling this endpoint.
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/market-data/public-get_book_summary_by_currency)
    """
    params: dict = {
      'currency': currency,
    }
    ...
    return await self.request(
      'public/get_book_summary_by_currency',
      params=params,
      validator=validate_get_book_summary_by_currency,
      validate=validate,
    )
