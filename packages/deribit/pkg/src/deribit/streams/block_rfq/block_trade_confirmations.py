"""`block_trade_confirmations` — subscription."""
...
validate_block_trade_confirmations = validator[BlockTradeConfirmation](
...
class BlockTradeConfirmations(StreamEndpoint):
  ...
  def block_trade_confirmations(
    ...
    *,
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/subscriptions/block-trade/block_trade_confirmations)
    """
    channel = f'block_trade_confirmations'
    return self.authed_subscribe(
      channel, validator=validate_block_trade_confirmations, validate=validate
    )
