"""`user.orders.{kind}.{currency}.{interval}` — subscription."""
...
validate_orders_by_kind_currency = validator[list[Order]](list[Order])
...
class OrdersByKindCurrency(StreamEndpoint):
  ...
  def orders_by_kind_currency(
    ...
    **Allowed values:** `raw`, `100ms`, `agg2`
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/subscriptions/user-orders-kind-currency-interval)
    """
    channel = f'user.orders.{kind}.{currency}.{interval}'
    return self.authed_subscribe(
      channel, validator=validate_orders_by_kind_currency, validate=validate
    )
