"""`user.orders.{kind}.{currency}.raw` — subscription."""
...
validate_orders_by_kind_currency_raw = validator[Order](Order)
...
class OrdersByKindCurrencyRaw(StreamEndpoint):
  ...
  def orders_by_kind_currency_raw(
    ...
    **Allowed values:** `BTC`, `ETH`, `USDC`, `USDT`, `EURR`, `any`
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/subscriptions/user-orders-kind-currency-raw)
    """
    channel = f'user.orders.{kind}.{currency}.raw'
    return self.authed_subscribe(
      channel, validator=validate_orders_by_kind_currency_raw, validate=validate
    )
