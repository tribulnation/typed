"""`user.combo_trades.{instrument_name}.{interval}` — subscription."""
...
class ComboTradesByInstrumentUpdate(TypedDict):
...
validate_combo_trades_by_instrument = validator[ComboTradesByInstrumentUpdate](
  ComboTradesByInstrumentUpdate
)
...
class ComboTradesByInstrument(StreamEndpoint):
  ...
  def combo_trades_by_instrument(
  ...
  ) -> StreamManager[ComboTradesByInstrumentUpdate, Any, Any]:
    ...
    **Allowed values:** `raw`, `100ms`, `agg2`
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/subscriptions/user-combo_trades-instrument_name-interval)
    """
    channel = f'user.combo_trades.{instrument_name}.{interval}'
    return self.authed_subscribe(
      channel, validator=validate_combo_trades_by_instrument, validate=validate
    )
