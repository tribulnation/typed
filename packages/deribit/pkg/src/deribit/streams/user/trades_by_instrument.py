"""`user.trades.{instrument_name}.{interval}` — subscription."""
...
class TradesByInstrumentUpdate(TypedDict):
  """Message schema for `user.trades.(instrument_name).(interval)` subscription - contains the data payload"""
...
validate_trades_by_instrument = validator[TradesByInstrumentUpdate](
  TradesByInstrumentUpdate
)
...
class TradesByInstrument(StreamEndpoint):
  ...
  def trades_by_instrument(
  ...
  ) -> StreamManager[TradesByInstrumentUpdate, Any, Any]:
    ...
    **Allowed values:** `raw`, `100ms`, `agg2`
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/subscriptions/user-trades-instrument_name-interval)
    """
    channel = f'user.trades.{instrument_name}.{interval}'
    return self.authed_subscribe(
      channel, validator=validate_trades_by_instrument, validate=validate
    )
