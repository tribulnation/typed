"""`user.combo_trades.{kind}.{currency}.{interval}` — subscription."""
...
class ComboTradesByKindCurrencyUpdate(TypedDict):
  """Message schema for `user.combo_trades.(kind).(currency).(interval)` subscription - contains the data payload"""
...
validate_combo_trades_by_kind_currency = validator[ComboTradesByKindCurrencyUpdate](
  ComboTradesByKindCurrencyUpdate
)
...
class ComboTradesByKindCurrency(StreamEndpoint):
  ...
  def combo_trades_by_kind_currency(
  ...
  ) -> StreamManager[ComboTradesByKindCurrencyUpdate, Any, Any]:
    ...
    **Allowed values:** `raw`, `100ms`, `agg2`
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/subscriptions/user-combo_trades-kind-currency-interval)
    """
    channel = f'user.combo_trades.{kind}.{currency}.{interval}'
    return self.authed_subscribe(
      channel, validator=validate_combo_trades_by_kind_currency, validate=validate
    )
