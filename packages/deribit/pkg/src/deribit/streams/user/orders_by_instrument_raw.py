"""`user.orders.{instrument_name}.raw` — subscription."""
...
validate_orders_by_instrument_raw = validator[Order](Order)
...
class OrdersByInstrumentRaw(StreamEndpoint):
  ...
  def orders_by_instrument_raw(
    ...
    *,
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/subscriptions/user-orders-instrument_name-raw)
    """
    channel = f'user.orders.{instrument_name}.raw'
    return self.authed_subscribe(
      channel, validator=validate_orders_by_instrument_raw, validate=validate
    )
