...
validate_get_order_state = validator[Order](Order)
...
class GetOrderState(RpcEndpoint):
  ...
  async def get_order_state(
    ...
    **Important Note for Mass Quotes:** Quote orders are order-like structures that don't fully translate to normal orders. When checking order state for quotes, the `amount` field represents the remaining amount, not the original order amount.
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/trading/private-get_order_state)
    """
    params: dict = {
      'order_id': order_id,
    }
    return await self.authed_request(
      'private/get_order_state',
      params=params,
      validator=validate_get_order_state,
      validate=validate,
    )
