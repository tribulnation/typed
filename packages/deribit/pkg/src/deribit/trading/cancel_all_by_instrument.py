...
validate_cancel_all_by_instrument = validator[float | list[CancelGroup]](float | list[CancelGroup])  # pyright: ignore[reportArgumentType]  # fmt: skip
...
class CancelAllByInstrument(RpcEndpoint):
  ...
  async def cancel_all_by_instrument(
    ...
    **📖 Related Article:** [Detailed Response for Cancel Methods](https://docs.deribit.com/articles/json-rpc-overview#detailed-response-for-cancel-methods)
    ...
    References:
      - [Deribit API docs](https://docs.deribit.com/api-reference/trading/private-cancel_all_by_instrument)
    """
    params: dict = {
    ...
    if include_combos is not None:
      params['include_combos'] = include_combos
    if freeze_quotes is not None:
      params['freeze_quotes'] = freeze_quotes
    return await self.authed_request(
      'private/cancel_all_by_instrument',
      params=params,
      validator=validate_cancel_all_by_instrument,
      validate=validate,
    )
