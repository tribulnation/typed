...
class GetBlockRfqTrades(RpcEndpoint):
  ...
  def get_block_rfq_trades_paged(
    ...
    *,
    ...
    Paged variant of `get_block_rfq_trades`: Passes each page's token back as `continuation` and stops when a response carries no `continuation`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(continuation: str) -> tuple[Sequence[BlockRfqTrade], str | None]:
