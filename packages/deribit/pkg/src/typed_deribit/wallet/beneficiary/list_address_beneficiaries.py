...
class ListAddressBeneficiaries(RpcEndpoint):
  ...
  def list_address_beneficiaries_paged(
    ...
    *,
    ...
    Paged variant of `list_address_beneficiaries`: Passes each page's token back as `continuation` and stops when a response carries no `continuation`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      continuation: str,
    ) -> tuple[Sequence[AddressBeneficiary], str | None]:
