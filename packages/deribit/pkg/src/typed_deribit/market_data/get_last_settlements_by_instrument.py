# Generated by typed-dev — see CONTRIBUTING.md.
from typed_core import PaginatedResponse
from typing_extensions import Literal, NotRequired, Sequence, TypedDict
from typed_deribit.core import TimestampMillis
from typed_deribit.core.endpoint.rpc import RpcEndpoint
from typed_deribit.schemas import Settlement, SettlementsPage


class Request(TypedDict):
  instrument_name: str
  """Unique instrument identifier"""
  type: NotRequired[Literal['settlement', 'delivery', 'bankruptcy']]
  """The type of settlement. `settlement`, `delivery` or `bankruptcy`."""
  count: NotRequired[int]
  """Number of requested items, default - `20`, maximum - `1000`"""
  continuation: NotRequired[str]
  """Continuation token for pagination"""
  search_start_timestamp: NotRequired[TimestampMillis]
  """The timestamp (milliseconds since the Unix epoch)"""


class GetLastSettlementsByInstrument(RpcEndpoint):
  """Retrieves historical settlement, delivery, and bankruptcy events for a specific instrument. Settlements occur when futures or options contracts expire and are settled at the delivery price.

  Results can be filtered by settlement type and timestamp. Use pagination parameters (`count` and `continuation`) to retrieve large settlement histories. This method is useful for tracking settlement history for a specific instrument.

  **Note on `profit_loss` and `session_profit_loss`:** Because this is a public endpoint, these fields are **platform-wide aggregates**, not per-account values. `profit_loss` is the sum of realized P&L of all position holders at the settlement price. `session_profit_loss` is the sum of each holder's total session P&L (realized + unrealized) across all participants."""

  def get_last_settlements_by_instrument_paged(
    self,
    *,
    instrument_name: str,
    type: Literal['settlement', 'delivery', 'bankruptcy'] | None = None,
    count: int | None = None,
    search_start_timestamp: TimestampMillis | None = None,
    transport: Literal['http', 'ws'] = 'http',
    validate: bool | None = None,
  ) -> PaginatedResponse[Settlement, str]:
    """Retrieves historical settlement, delivery, and bankruptcy events for a specific instrument. Settlements occur when futures or options contracts expire and are settled at the delivery price.

    Results can be filtered by settlement type and timestamp. Use pagination parameters (`count` and `continuation`) to retrieve large settlement histories. This method is useful for tracking settlement history for a specific instrument.

    **Note on `profit_loss` and `session_profit_loss`:** Because this is a public endpoint, these fields are **platform-wide aggregates**, not per-account values. `profit_loss` is the sum of realized P&L of all position holders at the settlement price. `session_profit_loss` is the sum of each holder's total session P&L (realized + unrealized) across all participants.

    Paged variant of `get_last_settlements_by_instrument`: Passes each page's token back as `continuation` and stops when a response carries no `continuation`. Awaitable (flattens every page) or async-iterable (one page at a time).

    Args:
      instrument_name: Unique instrument identifier
      type: The type of settlement. `settlement`, `delivery` or `bankruptcy`.
      count: Number of requested items, default - `20`, maximum - `1000`
      search_start_timestamp: The timestamp (milliseconds since the Unix epoch)
      validate: Override this call's response validation; falls back to the client-level default when omitted.
      transport: Transport to send this call over. Defaults to 'http'.

    References:
      - [Official docs](https://docs.deribit.com/api-reference/market-data/public-get_last_settlements_by_instrument)
    """

    async def next(continuation: str) -> tuple[Sequence[Settlement], str | None]:
      response = await self.get_last_settlements_by_instrument(
        instrument_name=instrument_name,
        type=type,
        count=count,
        search_start_timestamp=search_start_timestamp,
        transport=transport,
        continuation=(continuation or None),
        validate=validate,
      )
      rows = response.get('settlements')
      rows = rows if rows is not None else []
      state = response.get('continuation')
      return rows, state or None

    return PaginatedResponse('', next)

  async def get_last_settlements_by_instrument(
    self,
    *,
    instrument_name: str,
    type: Literal['settlement', 'delivery', 'bankruptcy'] | None = None,
    count: int | None = None,
    continuation: str | None = None,
    search_start_timestamp: TimestampMillis | None = None,
    validate: bool | None = None,
    transport: Literal['http', 'ws'] = 'http',
  ) -> SettlementsPage:
    """Retrieves historical settlement, delivery, and bankruptcy events for a specific instrument. Settlements occur when futures or options contracts expire and are settled at the delivery price.

    Results can be filtered by settlement type and timestamp. Use pagination parameters (`count` and `continuation`) to retrieve large settlement histories. This method is useful for tracking settlement history for a specific instrument.

    **Note on `profit_loss` and `session_profit_loss`:** Because this is a public endpoint, these fields are **platform-wide aggregates**, not per-account values. `profit_loss` is the sum of realized P&L of all position holders at the settlement price. `session_profit_loss` is the sum of each holder's total session P&L (realized + unrealized) across all participants.

    Args:
      instrument_name: Unique instrument identifier
      type: The type of settlement. `settlement`, `delivery` or `bankruptcy`.
      count: Number of requested items, default - `20`, maximum - `1000`
      continuation: Continuation token for pagination
      search_start_timestamp: The timestamp (milliseconds since the Unix epoch)
      validate: Override this call's response validation; falls back to the client-level default when omitted.
      transport: Transport to send this call over. Defaults to 'http'.

    References:
      - [Official docs](https://docs.deribit.com/api-reference/market-data/public-get_last_settlements_by_instrument)
    """
    request: Request = Request(instrument_name=instrument_name)
    if type is not None:
      request['type'] = type
    if count is not None:
      request['count'] = count
    if continuation is not None:
      request['continuation'] = continuation
    if search_start_timestamp is not None:
      request['search_start_timestamp'] = search_start_timestamp
    return await self.request(
      request,
      method='POST',
      path='public/get_last_settlements_by_instrument',
      meta={'public': True},
      validate=validate,
      transport=transport,
      request_type=Request,
      response_type=SettlementsPage,
    )
