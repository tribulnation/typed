  ...
  **📖 Related Support Article:** [Yield reward-bearing coins](https://support.deribit.com/hc/en-us/articles/31424939199261-Yield-reward-bearing-coins)"""
  ...
  def get_apr_history_paged(
    ...
    **📖 Related Support Article:** [Yield reward-bearing coins](https://support.deribit.com/hc/en-us/articles/31424939199261-Yield-reward-bearing-coins)
    ...
    Paged variant of `get_apr_history`: Passes each page's token back as `before` and stops when a response carries no `continuation`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(before: int) -> tuple[Sequence[AprDay], int | None]:
