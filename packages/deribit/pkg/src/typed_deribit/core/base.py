...
class ClientBase:
  ...
  async def __aenter__(self) -> Self:
    """Roll back acquired transports if a later entry fails or is cancelled."""
    async with AsyncExitStack() as stack:
      await stack.enter_async_context(self.rpc_client)
      await stack.enter_async_context(self.ws_client)
      stack.pop_all()
    return self

  async def __aexit__(self, exc_type, exc_value, traceback):
    """Attempt both exits, including when one raises or is cancelled."""
    stack = AsyncExitStack()
    stack.push_async_exit(self.rpc_client)
    stack.push_async_exit(self.ws_client)
