...
class GetTransactionLog(RpcEndpoint):
  ...
  def get_transaction_log_paged(
    ...
    *,
    ...
    Paged variant of `get_transaction_log`: Passes each page's token back as `continuation` and stops when a response carries no `continuation`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      continuation: int,
    ) -> tuple[Sequence[TransactionLogEntry], int | None]:
