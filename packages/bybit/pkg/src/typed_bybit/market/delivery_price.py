...
class DeliveryPriceEndpoint(RpcEndpoint):
  ...
  def delivery_price_paged(
    ...
    *,
    ...
    Paged variant of `delivery_price`: Passes each page's token back as `cursor` and stops when a response carries no `nextPageCursor`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(cursor: str) -> tuple[Sequence[DeliveryPrice], str | None]:
