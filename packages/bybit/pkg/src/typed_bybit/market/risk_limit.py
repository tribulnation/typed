...
class RiskLimit(RpcEndpoint):
  ...
  def risk_limit_paged(
    ...
    *,
    ...
    Paged variant of `risk_limit`: Passes each page's token back as `cursor` and stops when a response carries no `nextPageCursor`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(cursor: str) -> tuple[Sequence[RiskLimitTier], str | None]:
