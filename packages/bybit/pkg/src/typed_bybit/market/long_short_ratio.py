...
class LongShortRatioEndpoint(RpcEndpoint):
  ...
  def long_short_ratio_paged(
    ...
    *,
    ...
    Paged variant of `long_short_ratio`: Passes each page's token back as `cursor` and stops when a response carries no `nextPageCursor`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(cursor: str) -> tuple[Sequence[LongShortRatio], str | None]:
