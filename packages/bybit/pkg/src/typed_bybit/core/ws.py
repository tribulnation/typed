...
Every leaf sharing a connection shares the exact same `client` instance, since a
`Streams`' subscription bookkeeping lives on that one object -- there is no pool to hand
out fresh instances from the way `RpcEndpoint`'s shared `HttpTransport` is. `BybitBase`
(`core/base.py`) wires these primitives up with real URLs, credentials and the resolved
`streams`/`trade_ws` cores.
"""

from typing_extensions import Any, NotRequired, Self, TypedDict as _TypedDict, TypeVar, cast
...
from .http import BYBIT_DOMAINS, Region

T = TypeVar('T')

PLACEHOLDER = re.compile(r'\{([^{}]+)\}')
"""A `{name}` template slot inside a channel string -- design §7's location-marker
elimination: every one of Bybit's declared `parameters` fields is fully encoded into the
channel template this way (confirmed fleet-wide: no stream endpoint has a field left
over), so there is no separate subscribe-frame payload to send alongside it."""
...
class WsUrls(_TypedDict):
  ...
  trade: str
  fp: str
  """Advanced Earn's fixed-price offer channels (`finance_ws.*_offer`) -- named after the
  venue's own `/v5/public/fp` path segment, not the `finance_ws` function root, matching
...
class Frame(TypedDict):
  ...
  """`snapshot`/`delta` on a push (every public-category, spread and private push carries
  it, except `order_price_limit`); `COMMAND_RESP` on `option`'s subscribe ack. `parse_msg`
  forwards a push's value onto its `data` object, see there."""
...
class BybitStreamsClient(Streams[Any, Any, Reply, Reply]):
  ...
  def parse_msg(self, msg: str | bytes) -> Subscription[Any] | None:
    ...
    A push's payload is its `data` object, but Bybit states whether that payload is a
    full `snapshot` or a `delta` one level up, beside `topic`/`ts`, so unwrapping `data`
    alone dropped the one field an order-book or ticker consumer needs to tell a
    re-snapshot from an update. When `data` is a dict and the frame carries `type`, the
    frame's value is forwarded as `type` on the notification. A `type` key `data` already
    has wins (none does today, but a venue field must never be clobbered by a
    transport-added one), and a list-shaped `data` (kline, trade, insurance pool) is
    handed over untouched, since there is no object to put the field on.
    """
    ...
    if isinstance(data, dict) and 'type' in frame and 'type' not in data:
      data = {**data, 'type': frame['type']}
    return Subscription(channel=frame['topic'], notification=data)


@dataclass(kw_only=True, frozen=True)
class StreamsEndpoint:
  """Base class for every Bybit WebSocket subscription channel, over any of the eight
  public/private stream connections -- the resolved `streams` core (`codegen/config.toml`)."""

  client: BybitStreamsClient
  validate: bool = True
  ...
  def subscribe(
    ...
    *,
  ...
  ) -> StreamManager[T, Reply, Reply]:
    """One channel subscription (design §2/§8's `subscribe` verb).

    `channel` is the raw template as declared in the spec (e.g.
    `orderbook.{depth}.{symbol}`) -- substituting its `{name}` placeholders from
    `request`'s own wire-serialized fields is this connection's own job, matching every
    other wire-mechanics decision `core` already owns (design §7). Every field Bybit
    declares on a subscribe call is fully encoded into the channel template this way, so
    there is no separate subscribe-frame payload left to send alongside it.

    Bybit's transport already unwraps every push's envelope before a caller sees it
    (`BybitStreamsClient.parse_msg` forwards `data`, plus the frame's `snapshot`/`delta`
    `type` when `data` is an object), so `response_type` validates the pushed payload
    directly.

    Args:
      channel: The wire channel name template, e.g. `orderbook.{depth}.{symbol}`.
    ...
    manager = self.client.subscribe(resolved_channel)
    payload_validator = (
      validator(cast(type, response_type)) if response_type is not None else None
    )
    if payload_validator is None or not self.should_validate(validate):
      return cast(StreamManager[T, Reply, Reply], manager)
    return manager.map(payload_validator)
...
class BybitTradeClient(Rpc[Any, TradeReply]):
  ...
  live-verified twice (`spec/endpoints/trade_ws/ping`), including once with an explicit
...
@dataclass(kw_only=True, frozen=True)
class TradeEndpoint:
  """Base class for every Bybit WS Trade command -- the resolved `trade_ws` core
  (`codegen/config.toml`). `request()` builds and signs the command frame it would send, and
  returns that frame directly, without ever sending it -- a deliberate, pre-existing
  safety property of this connection (`BybitTradeClient.build_request` performs no
  network I/O at all), not something this migration changes."""
  ...
  async def request(
    ...
    *,
    ...
    """Build the signed `path` command frame and return it, without sending it (see the
    class docstring). `validate`/`response_type` are accepted so every generated `rpc`
    method shares one call shape (S8) -- there is no venue reply here to validate. No
    `meta` parameter -- the resolved `trade_ws` core declares no `meta` schema in
    `codegen/config.toml` (every WS Trade command signs unconditionally), so no generated call
    ever passes one (design §6: "a core with no `meta` schema means every endpoint
    resolving to it must declare `meta: {}`" -- and generates with no `meta=` argument
    at all, matching kraken's `SocketEndpoint`, the identical precedent for a core with
    no declared `meta` schema).

    Args:
      request: The generated `Request` value (a `TypedDict` instance, or `None`).
      path: The wire command name, e.g. `order.create`.
      validate: Unused, see the class docstring.
      request_type: Unused -- the command frame's own `args` element is the raw
        `request` value itself, not a validated/serialized one (matching this
        connection's pre-existing behavior).
      response_type: Unused, see the class docstring.
    """
    return cast(T, self.client.build_request(path, [request if request is not None else {}]))
