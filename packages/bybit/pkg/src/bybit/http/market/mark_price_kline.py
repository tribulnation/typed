"""`GET /v5/market/mark-price-kline` — Get Mark Price Kline."""
...
class MarkPriceKlineResult(TypedDict):
...
adapter = validator[MarkPriceKlineResult](MarkPriceKlineResult)


class MarkPriceKline(Endpoint):
  """`Get Mark Price Kline` — mixed into the router that owns `market.mark_price_kline`."""

  async def mark_price_kline(
    self,
    *,
    category: Literal['linear', 'inverse', 'option'] | None = None,
    symbol: str,
    ...
    """Get mark price candles for a contract. Candles are returned newest first.
    ...
    Args:
      ...
      limit: Number of candles per page. Range [1, 1000] for futures and [1, 500] for options; defaults to 200.
      validate: Validate the response against the generated schema.

    References:
      - [Bybit API docs](https://bybit-exchange.github.io/docs/v5/market/mark-kline)
    """
    params: dict = {
      'symbol': symbol,
      'interval': interval,
    ...
    if limit is not None:
      params['limit'] = limit
    r = await self.request('GET', '/v5/market/mark-price-kline', params=params)
    return self.result(r, adapter, validate)

  async def mark_price_kline_paged(
    self,
    *,
    category: Literal['linear', 'inverse', 'option'] | None = None,
    symbol: str,
    ...
    validate: bool | None = None,
  ) -> AsyncIterator[MarkPriceKlineResult]:
    """Yield successive pages of `mark_price_kline`.
    ...
    if start is None or end is None:
      raise ValueError(
        '`mark_price_kline_paged` walks a time window: pass both `start` and `end`'
      )
    lower = start
    upper = end
    width = upper - lower
    pages = 0
    while True:
      response = await self.mark_price_kline(
        category=category,
        symbol=symbol,
        interval=interval,
        start=lower,
      ...
      if not allow_truncation and len(rows) >= (limit if limit is not None else 200):
        raise LogicError(
          f'`mark_price_kline_paged` requested the window {lower} to {upper} and the venue returned a full page of {len(rows)} rows, so it may hold more; advancing would move past the rows that were left out. Narrow the window, or pass `allow_truncation=True` to accept the loss.'
        )
      upper = lower - 1
      lower = upper - width
