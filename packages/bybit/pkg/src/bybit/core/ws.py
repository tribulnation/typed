...
Bybit's v5 WebSocket surface is six separate connections, not one: four public
category streams (`spot`, `linear`, `inverse`, `option`), one private stream, and one
order-entry ("WS Trade") connection — see
...
class Frame(TypedDict):
  ...
  `{"args": [...], "op": "pong"}` on option/spread. This type only proves a frame is
  well-formed enough to route; `Reply` re-validates the stricter ack shape once a
  frame is confirmed to be one, and `is_pong` (not this shape) decides that.
  """
...
def is_pong(frame: Frame) -> bool:
  ...
  signal instead. Checking both covers every documented variant."""
...
class BybitTradeClient(Rpc[Any, TradeReply]):
  ...
  def parse_response(self, msg: str | bytes) -> Response[TradeReply] | None:
    """Drop pongs and anything else without a `reqId` before validating — only a
    genuine command reply is shaped like `TradeReply`."""
