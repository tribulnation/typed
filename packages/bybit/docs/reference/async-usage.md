...
`client.http` (one shared HTTP connection pool) and all eight of `client.ws`'s WebSocket
connections (`spot`, `linear`, `inverse`, `option`, `spread`, `rfq`, `private`, `trade`) open
concurrently, and every one of them closes cleanly on exit — the caller never separately
enters `client.http` or an individual `client.ws.*` connection.
...
Bybit's WebSocket surface is eight separate connections, not one: six public-category
channels (`spot`, `linear`, `inverse`, `option`, `spread`, `rfq`), one private channel
(`private`), and one order-entry connection (`trade` — see below, it isn't a subscription).
