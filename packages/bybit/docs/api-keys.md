...
The v5 Market REST surface (`client.market`) and the public-category WebSocket streams
(`client.spot`, `.linear`, `.inverse`, `.option`, `.spread`, `.rfq`, `.finance`) are open
...
client, restricted to `client.market` and the public WebSocket streams:
...
`client.trade`, `.position`, `.account`, `.asset`, and the rest of the authenticated REST
tree, the signed `private` WebSocket connection (`client.private`), and the signed WS Trade
order-entry connection (`client.trade_ws`).
