...
`limit` rows, and Bybit keeps the newest ones when a range holds more. `kline_paged` walks a
longer range for you, moving `end` back to the earliest candle of each full page until it
reaches `start`; `await` it for every candle in one list. See
