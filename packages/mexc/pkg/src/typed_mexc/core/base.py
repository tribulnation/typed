...
from typed_mexc.futures.core import (
...
class MexcBase:
      ...
      futures_clients=FuturesClients(
        ...
        streams_client=FuturesStreamsClients(
          ...
          user_client=FuturesPrivateStreamsClient(
            credentials=credentials, url=futures_ws_url
          ),
        ),
      ),
    )
  ...
  async def __aenter__(self) -> Self:
    await asyncio.gather(
      self.spot_clients.__aenter__(), self.futures_clients.__aenter__()
    )
    return self

  async def __aexit__(self, exc_type, exc_value, traceback):
    """Close dependents first and attempt every exit even when one fails."""
    stack = AsyncExitStack()
    stack.push_async_exit(self.spot_clients)
    stack.push_async_exit(self.futures_clients)
