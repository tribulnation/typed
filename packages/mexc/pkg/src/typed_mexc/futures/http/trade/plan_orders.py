...
class PlanOrders(FuturesHttpEndpoint):
  ...
  def plan_orders_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[PlanOrdersItem, int]:
    """Returns paginated trigger/plan orders for the signed futures account.

    Paged variant of `plan_orders`: Requests `page_num` from 1 upwards and stops on the first page shorter than `page_size`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_num: int) -> tuple[Sequence[PlanOrdersItem], int | None]:
