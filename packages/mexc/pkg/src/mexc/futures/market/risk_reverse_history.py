...
class RiskReverseHistory(FuturesMixin):
  async def risk_reverse_history(
    self, *,
    symbol: str, page_num: int, page_size: int, validate: bool | None = None,
  ...
  async def risk_reverse_history_paged(
    self, *,
    symbol: str, page_size: int, max_pages: int | None = None,
    ...
    pages = 0
    while True:
      response = await self.risk_reverse_history(symbol=symbol, page_size=page_size, page_num=page_num, validate=validate)
      yield response
      pages += 1
