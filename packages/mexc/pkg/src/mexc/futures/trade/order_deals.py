...
class OrderDeals(AuthFuturesMixin):
  async def order_deals(
    ...
    end_time: Timestamp | None = None, page_num: int, page_size: int,
  ...
  async def order_deals_paged(
    self, *,
    symbol: str, start_time: Timestamp | None = None,
    end_time: Timestamp | None = None, page_size: int,
    ...
    pages = 0
    while True:
      response = await self.order_deals(symbol=symbol, start_time=start_time, end_time=end_time, page_size=page_size, page_num=page_num, validate=validate)
      yield response
      pages += 1
