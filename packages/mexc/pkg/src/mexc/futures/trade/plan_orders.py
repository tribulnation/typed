...
class PlanOrders(AuthFuturesMixin):
  ...
  async def plan_orders_paged(
    ...
    pages = 0
    while True:
      response = await self.plan_orders(symbol=symbol, states=states, start_time=start_time, end_time=end_time, page_size=page_size, page_num=page_num, validate=validate)
      yield response
      pages += 1
