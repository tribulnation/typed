...
Response: type[list[tuple[Timestamp, str, str, str, str, str, Timestamp, str]] | ErrorResponse] = list[tuple[Timestamp, str, str, str, str, str, Timestamp, str]] | ErrorResponse # type: ignore
...
class Candles(SpotMixin):
  ...
  ) -> list[tuple[Timestamp, str, str, str, str, str, Timestamp, str]]:
  ...
  async def candles_paged(
  ...
  ) -> AsyncIterator[list[tuple[Timestamp, str, str, str, str, str, Timestamp, str]]]:
    ...
    if start_time is None or end_time is None:
      raise ValueError('`candles_paged` walks a time window: pass both `start_time` and `end_time`')
    lower = start_time
    upper = end_time
    width = upper - lower
    pages = 0
    while True:
      response = await self.candles(symbol=symbol, interval=interval, start_time=lower, end_time=upper, limit=limit, validate=validate)
      yield response
      pages += 1
