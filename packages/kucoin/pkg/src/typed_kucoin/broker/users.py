...
class Users(RpcEndpoint):
  ...
  def users_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[BrokerUser, int]:
    """Page through the authenticated API Broker's onboarded users, with their lifetime trading volume, fee and commission -- broken out by whether the trades carried a broker tag -- optionally filtered by referral code, tag or registration date.

    Paged variant of `users`: Requests `page` from 1 upwards and stops once it has covered the `totalPage` pages the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[BrokerUser], int | None]:
