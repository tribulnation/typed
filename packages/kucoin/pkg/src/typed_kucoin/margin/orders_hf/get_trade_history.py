...
class GetTradeHistory(RpcEndpoint):
  ...
  def get_trade_history_paged(
    ...
    *,
    ...
    Paged variant of `get_trade_history`: Passes each page's token back as `lastId` and stops when a response carries no `lastId`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(last_id: int) -> tuple[Sequence[HfFill], int | None]:
