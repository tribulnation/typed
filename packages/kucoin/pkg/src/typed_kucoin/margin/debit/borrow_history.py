# Generated by typed-dev — see CONTRIBUTING.md.
from decimal import Decimal
from typed_core import PaginatedResponse
from typing_extensions import Literal, NotRequired, Sequence, TypedDict
from typed_kucoin.core import TimestampMillis
from typed_kucoin.core.endpoint.rpc import RpcEndpoint


class BorrowHistoryEntry(TypedDict):
  orderNo: str
  """Unique borrow order id."""
  symbol: str
  """Isolated-margin trading pair; the literal `DEFAULT` for a cross-margin borrow, not `null` as the docs describe (confirmed live)."""
  currency: str
  """Borrowed currency."""
  size: Decimal
  """Requested borrow amount."""
  actualSize: Decimal
  """Actual borrowed amount."""
  status: Literal['PENDING', 'SUCCESS', 'FAILED']
  """Borrow order status."""
  createdTime: TimestampMillis
  """Unix milliseconds this borrow was initiated."""


class Request(TypedDict):
  currency: NotRequired[str]
  """Currency to filter by, e.g. `BTC`, `ETH`, `KCS`. Queries all currencies if omitted."""
  isIsolated: NotRequired[bool]
  """Whether to look at isolated-margin (`true`) or cross-margin (`false`) borrows."""
  symbol: NotRequired[str]
  """Trading pair, e.g. `BTC-USDT`. Mandatory for an isolated-margin account."""
  orderNo: NotRequired[str]
  """Filter by a specific borrow order id."""
  startTime: NotRequired[TimestampMillis]
  """Start of the time window, Unix milliseconds. Unrestricted; if omitted or below 1680278400000 (2023-04-01T00:00:00Z), that value is used instead."""
  endTime: NotRequired[TimestampMillis]
  """End of the time window, Unix milliseconds."""
  currentPage: NotRequired[int]
  """Page number."""
  pageSize: NotRequired[int]
  """Results per page."""


class BorrowHistoryPage(TypedDict):
  """One page of cross/isolated margin borrow orders."""

  timestamp: TimestampMillis
  """Server timestamp, Unix milliseconds."""
  currentPage: int
  """Current page number."""
  pageSize: int
  """Results per page, as requested."""
  totalNum: int
  """Total matching records."""
  totalPage: int
  """Total number of pages."""
  items: list[BorrowHistoryEntry]
  """Borrow orders on this page."""


class BorrowHistory(RpcEndpoint):
  """Page through this account's cross- or isolated-margin borrow orders, newest first."""

  def borrow_history_paged(
    self,
    *,
    currency: str | None = None,
    is_isolated: bool | None = None,
    symbol: str | None = None,
    order_no: str | None = None,
    start_time: TimestampMillis | None = None,
    end_time: TimestampMillis | None = None,
    page_size: int | None = None,
    validate: bool | None = None,
  ) -> PaginatedResponse[BorrowHistoryEntry, int]:
    """Page through this account's cross- or isolated-margin borrow orders, newest first.

    Paged variant of `borrow_history`: Requests `currentPage` from 1 upwards and stops once it has covered the `totalPage` pages the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).

    Args:
      currency: Currency to filter by, e.g. `BTC`, `ETH`, `KCS`. Queries all currencies if omitted.
      is_isolated: Whether to look at isolated-margin (`true`) or cross-margin (`false`) borrows.
      symbol: Trading pair, e.g. `BTC-USDT`. Mandatory for an isolated-margin account.
      order_no: Filter by a specific borrow order id.
      start_time: Start of the time window, Unix milliseconds. Unrestricted; if omitted or below 1680278400000 (2023-04-01T00:00:00Z), that value is used instead.
      end_time: End of the time window, Unix milliseconds.
      page_size: Results per page.
      validate: Override this call's response validation; falls back to the client-level default when omitted.

    References:
      - [Official docs](https://www.kucoin.com/docs-new/api-3470207)
    """

    async def next(
      current_page: int,
    ) -> tuple[Sequence[BorrowHistoryEntry], int | None]:
      response = await self.borrow_history(
        currency=currency,
        is_isolated=is_isolated,
        symbol=symbol,
        order_no=order_no,
        start_time=start_time,
        end_time=end_time,
        page_size=page_size,
        current_page=current_page,
        validate=validate,
      )
      rows = response.get('items')
      rows = rows if rows is not None else []
      total = response.get('totalPage')
      total = int(total) if total is not None else None
      if not rows or (total is not None and (current_page - 1 + 1) >= total):
        return rows, None
      return rows, current_page + 1

    return PaginatedResponse(1, next)

  async def borrow_history(
    self,
    *,
    currency: str | None = None,
    is_isolated: bool | None = None,
    symbol: str | None = None,
    order_no: str | None = None,
    start_time: TimestampMillis | None = None,
    end_time: TimestampMillis | None = None,
    current_page: int | None = None,
    page_size: int | None = None,
    validate: bool | None = None,
  ) -> BorrowHistoryPage:
    """Page through this account's cross- or isolated-margin borrow orders, newest first.

    Args:
      currency: Currency to filter by, e.g. `BTC`, `ETH`, `KCS`. Queries all currencies if omitted.
      is_isolated: Whether to look at isolated-margin (`true`) or cross-margin (`false`) borrows.
      symbol: Trading pair, e.g. `BTC-USDT`. Mandatory for an isolated-margin account.
      order_no: Filter by a specific borrow order id.
      start_time: Start of the time window, Unix milliseconds. Unrestricted; if omitted or below 1680278400000 (2023-04-01T00:00:00Z), that value is used instead.
      end_time: End of the time window, Unix milliseconds.
      current_page: Page number.
      page_size: Results per page.
      validate: Override this call's response validation; falls back to the client-level default when omitted.

    References:
      - [Official docs](https://www.kucoin.com/docs-new/api-3470207)
    """
    request: Request = Request()
    if currency is not None:
      request['currency'] = currency
    if is_isolated is not None:
      request['isIsolated'] = is_isolated
    if symbol is not None:
      request['symbol'] = symbol
    if order_no is not None:
      request['orderNo'] = order_no
    if start_time is not None:
      request['startTime'] = start_time
    if end_time is not None:
      request['endTime'] = end_time
    if current_page is not None:
      request['currentPage'] = current_page
    if page_size is not None:
      request['pageSize'] = page_size
    return await self.request(
      request,
      method='GET',
      path='/api/v3/margin/borrow',
      meta={'signed': True},
      validate=validate,
      request_type=Request,
      response_type=BorrowHistoryPage,
    )
