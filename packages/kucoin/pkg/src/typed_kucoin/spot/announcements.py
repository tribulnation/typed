...
class Announcements(RpcEndpoint):
  ...
  def announcements_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[Announcement, int]:
    """Page through KuCoin's news announcements (new listings, delistings, maintenance, activities, ...), newest first. Default search window covers the last month unless `startTime`/`endTime` narrow it.

    Paged variant of `announcements`: Requests `currentPage` from 1 upwards and stops once it has covered the `totalPage` pages the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(current_page: int) -> tuple[Sequence[Announcement], int | None]:
