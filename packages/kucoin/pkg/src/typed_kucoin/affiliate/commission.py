...
class Commission(RpcEndpoint):
  ...
  def commission_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[AffiliateCommission, int]:
    """Page through this affiliate account's own commission records: per-period payout, trading volume and rebate, for either trade-based or Kumining-based commission.

    Paged variant of `commission`: Requests `page` from 1 upwards and stops on the first page shorter than `page_size`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[AffiliateCommission], int | None]:
      ...
      rows = response if response is not None else []
      if not rows or len(rows) < (min(page_size, 500) if page_size is not None else 10):
        return rows, None
      return rows, page + 1
