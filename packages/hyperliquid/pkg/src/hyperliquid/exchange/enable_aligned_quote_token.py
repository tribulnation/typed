...
class EnableAlignedQuoteTokenAction(TypedDict):
  type: Literal['spotDeploy']
  enableAlignedQuoteToken: int
...
class EnableAlignedQuoteToken(ExchangeMixin):
  async def enable_aligned_quote_token(
    ...
    *,
    ...
    """
    ts = timestamp.now()
    action: EnableAlignedQuoteTokenAction = {
      'type': 'spotDeploy',
      'enableAlignedQuoteToken': token,
    }
    sig = sign_l1_action(
      action,
      wallet=self.wallet,
