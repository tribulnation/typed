...
Almost every `info.*` operation posts the same shape to one fixed surface: a flat JSON
object carrying a `type` discriminator plus that action's own fields, with no further
wire-placement decision to make (design §2/§7) -- `path` (`RpcEndpointSpec.path`, e.g.
...
discriminator), never a URL to route on. `_WRAPPED_REQUEST` below is the one confirmed
exception.
"""
...
_WRAPPED_REQUEST: frozenset[str] = frozenset({'candleSnapshot'})
"""`info.*` wire types whose request fields nest under a `req` key instead of sitting
flat beside `type` -- confirmed against this endpoint's own `upstream.md`
(`{"type": "candleSnapshot", "req": {"coin": ..., "interval": ..., "startTime": ...,
"endTime": ...}}`) and live: the flat shape every other `info.*` operation uses (`{"type":
"candleSnapshot", "coin": ..., ...}`) gets a `422` ("Failed to deserialize the JSON body
into the target type") from both api.hyperliquid.xyz and api.hyperliquid-testnet.xyz. The
spec's own `request` schema stays flat (`coin`/`endTime`/`interval`/`startTime` as
top-level properties) so the generated method keeps its flat, ergonomic keyword signature
matching every sibling `info.*` call -- only this wire-assembly step differs."""
...
class InfoCore(RpcEndpoint):
  ...
  async def request(
    ...
    *,
    ...
    wire as `{"type": path, ...fields}` (or `{"type": path, "req": {...fields}}` for the
    one endpoint in `_WRAPPED_REQUEST`), POST or send it over the requested transport, and
    ...
    payload = (
      {'type': path, 'req': values}
      if path in _WRAPPED_REQUEST
      else {
        'type': path,
        **values,
      }
  ...
  def new(
    ...
    client: RpcClient,
    *,
    info_ws_client: RpcClient | None = None,
    validate: bool = True,
  ) -> Self:
