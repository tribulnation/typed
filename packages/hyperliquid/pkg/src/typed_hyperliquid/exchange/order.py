...
from typed_hyperliquid.schemas import ExchangeError
...
class PlaceOrderOk(TypedDict):
  """The venue accepted the action as a whole; check each entry of `response.data.statuses` for per-order outcomes, which may still carry an individual `error`."""

  status: Literal['ok']
  """The venue accepted the action as a whole."""
  response: OrderActionResult


PlaceOrderResponse = PlaceOrderOk | ExchangeError
