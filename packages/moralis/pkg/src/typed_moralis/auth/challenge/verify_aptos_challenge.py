...
class VerifyAptosChallengeRequest(TypedDict):
  message: str
...
class VerifyAptosChallengeResponse(TypedDict):
  id: str
...
verify_aptos_challenge_adapter = TypeAdapter(VerifyAptosChallengeResponse)


class VerifyAptosChallenge(RpcEndpoint):
  async def verify_aptos_challenge(
    self,
    verify_aptos_challenge_request: VerifyAptosChallengeRequest,
    *,
    validate: bool | None = None,
  ) -> VerifyAptosChallengeResponse:
    """Verify Aptos challenge

    Args:
      verify_aptos_challenge_request: Verify Aptos challenge message.
      validate: Per-call response validation override.

    References:
      - [Moralis API docs](https://docs.moralis.com/api-reference/challenge/verify-aptos-challenge)
    """
    response = await self.request(
      'POST',
      '/challenge/verify/aptos',
      json=verify_aptos_challenge_request,
    )
    if self.should_validate(validate):
      return verify_aptos_challenge_adapter.validate_json(response.text)
