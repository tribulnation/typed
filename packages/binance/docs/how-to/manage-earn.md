...
Flexible product; Locked products (`client.spot.simple_earn.locked`) follow the same shape
...
`client.spot.simple_earn.locked` follows the same list/subscribe/position/redeem shape for
Locked products. `client.spot.staking` (ETH, SOL, and on-chain-yields staking) covers its own
products with `stake`/`redeem`/`account`/`quota` instead — e.g.
`client.spot.staking.eth.stake(amount=0.1)`.
