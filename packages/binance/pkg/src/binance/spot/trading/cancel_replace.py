  ...
  async def cancel_replace(
    ...
    *,
    ...
    Args:
      ...
      cancel_orig_client_order_id: Client order id of the order to cancel. Either cancelOrderId or cancelOrigClientOrderId must be sent; when both are sent, cancelOrderId is searched first and cancelOrigClientOrderId is then checked against that order, rejecting the request if it does not match.
      cancel_order_id: Id of the order to cancel. Either cancelOrderId or cancelOrigClientOrderId must be sent; see cancelOrigClientOrderId for the lookup order when both are sent.
      new_client_order_id: Caller-supplied id for the new order.
      ...
      order_rate_limit_exceeded_mode: How to proceed when the account has exceeded the unfilled order rate limit. DO_NOTHING (default) attempts the cancel only if the limit has not been exceeded; CANCEL_ONLY always cancels the order.
