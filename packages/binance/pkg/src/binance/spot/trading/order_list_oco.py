...
class OrderListOcoOrderReportAck(TypedDict):
...
class OrderListOcoOrderReportFill(TypedDict):
...
class OrderListOcoOrderReportResult(TypedDict):
...
class OrderListOcoOrderReportFull(TypedDict):
  ...
  preventedQuantity: NotRequired[str]
  """Order quantity that expired due to self-trade prevention. Present only when the order expired due to self-trade prevention."""
  fills: list[OrderListOcoOrderReportFill]
...
class OrderListOcoResult(TypedDict):
  ...
  orderReports: list[
    OrderListOcoOrderReportAck
    | OrderListOcoOrderReportResult
    | OrderListOcoOrderReportFull
  ]
  """Per-leg order acknowledgements. Each entry's shape is discriminated by the request's newOrderRespType parameter."""


class OrderListOco(RpcEndpoint):
  """Send in a one-cancels-the-other (OCO) pair, where activation of one order immediately cancels the other. One of the two legs (the above order) must be LIMIT_MAKER, TAKE_PROFIT or TAKE_PROFIT_LIMIT, and the other (the below order) must be STOP_LOSS or STOP_LOSS_LIMIT. Price restrictions: on the SELL side, above price > last traded price > below trigger price; on the BUY side, above price < last traded price < below trigger price. An OCO adds 2 orders to the EXCHANGE_MAX_ORDERS and MAX_NUM_ORDERS filters."""

  async def order_list_oco(
    self,
    *,
    symbol: str,
