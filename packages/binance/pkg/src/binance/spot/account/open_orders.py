...
class OpenOrders(RpcEndpoint):
  ...
  async def open_orders(
    ...
    *,
    ...
    """Get all open orders on a symbol. If `symbol` is omitted, open orders for all symbols are returned as an array — careful when accessing this with no symbol: request weight is 6 for a single symbol but jumps to 80 when `symbol` is omitted.

    Args:
      symbol: Symbol to query. If omitted, open orders for all symbols are returned as an array. Weight is 6 for a single symbol, 80 when omitted.
