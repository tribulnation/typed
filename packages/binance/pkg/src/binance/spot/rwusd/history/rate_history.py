...
class RwusdRateHistoryEntry(TypedDict):
...
class RwusdRateHistoryPage(TypedDict):
  """One page of RWUSD's historical annual percentage rate."""

  rows: NotRequired[list[RwusdRateHistoryEntry]]
...
class RateHistory(RpcEndpoint):
  """Get RWUSD's historical annual percentage rate, sorted descending by time."""
  ...
  async def rate_history(
    ...
    *,
    ...
    References:
      - [Official docs](https://developers.binance.com/en/docs/catalog/investment-and-services-simple-earn/api/rest-api/rwusd#get-rwusd-rate-history)
    """
    params = {}
    ...
    if size is not None:
      params['size'] = size
    _Response = RwusdRateHistoryPage
  ...
  async def rate_history_paged(
    ...
    *,
    ...
    validate: bool | None = None,
  ) -> AsyncIterator[RwusdRateHistoryPage]:
