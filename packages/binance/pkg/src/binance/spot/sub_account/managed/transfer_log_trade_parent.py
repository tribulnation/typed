...
class TransferLogTradeParent(RpcEndpoint):
  """Query a Managed Sub-Account's transfer log, for the trading team's master account."""

  async def transfer_log_trade_parent(
    self,
    *,
    email: str,
    start_time: int,
    ...
    References:
      - [Official docs](https://developers.binance.com/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/managed-sub-account#query-managed-sub-account-transfer-log-master-account-trading)
    """
    params: dict = {
      'email': email,
  ...
  async def transfer_log_trade_parent_paged(
    self,
    *,
    email: str,
    start_time: int,
    ...
    """Yield successive pages of `transfer_log_trade_parent`.
    ...
    pages = 0
    while True:
      response = await self.transfer_log_trade_parent(
        email=email,
