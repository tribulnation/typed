...
class CanceledOrderEntry(TypedDict):
  """A single canceled order, not part of an order list."""
...
class OpenOrdersCancelAll(WsRpcEndpoint):
  ...
  async def open_orders_cancel_all(
    ...
    *,
    ...
    """Cancel all open orders on a symbol, including orders that are part of an order list.

    Args:
      symbol: Trading pair symbol.

    References:
      - [Official docs](https://github.com/binance/binance-spot-api-docs/blob/master/web-socket-api.md#cancel-open-orders-trade)
    """
    params: dict = {
      'symbol': symbol,
    }
    ...
    return await self.authed_request(
      'openOrders.cancelAll', params=params, validator=_validator, validate=validate
    )
