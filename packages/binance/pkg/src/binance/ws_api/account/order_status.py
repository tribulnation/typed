...
class OrderStatusEndpoint(WsRpcEndpoint):
  """Query order"""
  ...
  async def order_status(
    ...
    *,
    ...
    """Check execution status of an order. Either `orderId` or `origClientOrderId` must be sent; if both are provided, `orderId` is searched first, then `origClientOrderId` is checked against that result -- the request is rejected if neither condition is met.
    ...
    References:
      - [Official docs](https://github.com/binance/binance-spot-api-docs/blob/master/web-socket-api.md#query-order-user_data)
    """
    params: dict = {
      'symbol': symbol,
    }
