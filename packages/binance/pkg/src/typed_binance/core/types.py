...
000+00:00`, already UTC-offset), and the `YYYY-MM-DD[ HH:MM:SS]`-shaped fields on
withdraw-history and fund-NAV-history endpoints (a space, not `T`, and no offset at all,
but documented as UTC). `IsoConverter`'s `tz` field (typed-core 0.8.0) attaches UTC to an
offset-free value by default, which is exactly the treatment the second group needs, so no
subclass is required; `datetime.fromisoformat` parses both shapes cleanly."""
