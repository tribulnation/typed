...
class HttpRpcClient(RpcClient):
  ...
  async def keyed_request(
    ...
    *,
    ...
    """Send one request carrying the `X-MBX-APIKEY` header but no signature.

    Binance's `MARKET_DATA`/`USER_STREAM` security tiers (and a couple of `USER_DATA`-
    tagged endpoints that genuinely document no signature — see `spec/core.md`'s
    Authentication section) need the API key present to identify the caller, but never add
    `timestamp`/`recvWindow`/`signature` — those only apply to a truly signed request.
