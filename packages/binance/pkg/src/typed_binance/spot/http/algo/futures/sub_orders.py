...
class SubOrders(RpcEndpoint):
  ...
  def sub_orders_paged(
    ...
    *,
    ...
    Paged variant of `sub_orders`: Requests `page` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[FuturesAlgoSubOrder], int | None]:
