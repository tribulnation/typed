...
class Orders(RpcEndpoint):
  ...
  def orders_paged(
    ...
    *,
    ...
    Paged variant of `orders`: Requests `page` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[FiatOrder], int | None]:
