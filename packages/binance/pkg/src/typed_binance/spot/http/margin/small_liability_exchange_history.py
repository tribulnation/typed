...
class SmallLiabilityExchangeHistory(RpcEndpoint):
  ...
  def small_liability_exchange_history_paged(
    ...
    *,
    ...
    Paged variant of `small_liability_exchange_history`: Requests `current` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      current: int,
    ) -> tuple[Sequence[MarginSmallLiabilityExchangeRecord], int | None]:
