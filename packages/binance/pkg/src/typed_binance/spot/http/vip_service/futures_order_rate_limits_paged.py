...
class FuturesOrderRateLimitsPaged(RpcEndpoint):
  ...
  def futures_order_rate_limits_paged_paged(
    ...
    *,
    ...
    Paged variant of `futures_order_rate_limits_paged`: Requests `pageIndex` from 1 upwards and stops once it has covered the `data.total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      page_index: int,
    ) -> tuple[Sequence[FuturesOrderRateLimitPageEntry], int | None]:
