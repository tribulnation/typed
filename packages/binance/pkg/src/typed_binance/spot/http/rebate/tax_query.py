...
class TaxQuery(RpcEndpoint):
  ...
  def tax_query_paged(
    ...
    *,
    ...
    Paged variant of `tax_query`: Requests `page` from 1 upwards and stops once it has covered the `data.totalPageNum` pages the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[RebateRecord], int | None]:
