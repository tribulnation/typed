...
class AccountEarning(RpcEndpoint):
  ...
  def account_earning_paged(
    ...
    *,
    ...
    Paged variant of `account_earning`: Requests `pageIndex` from 1 upwards and stops once it has covered the `data.totalNum` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      page_index: int,
    ) -> tuple[Sequence[MiningAccountEarningRecord], int | None]:
