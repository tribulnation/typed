...
class FuturesMovePositionHistory(RpcEndpoint):
  ...
  def futures_move_position_history_paged(
    ...
    *,
    ...
    Paged variant of `futures_move_position_history`: Requests `page` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      page: int,
    ) -> tuple[Sequence[SubAccountMovePositionRecord], int | None]:
      ...
      if not rows_ or (total is not None and (page - 1 + 1) * rows >= total):
        return rows_, None
      return rows_, page + 1
