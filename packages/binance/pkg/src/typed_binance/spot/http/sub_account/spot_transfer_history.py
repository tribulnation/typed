...
class SpotTransferHistory(RpcEndpoint):
  ...
  def spot_transfer_history_paged(
    self,
    *,
  ...
  ) -> PaginatedResponse[SubAccountSpotTransfer, int]:
    """Query the history of spot asset transfers between sub-accounts (or between a sub-account and the master account), for the master account.

    Paged variant of `spot_transfer_history`: Requests `page` from 1 upwards and stops on the first page shorter than `limit`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[SubAccountSpotTransfer], int | None]:
      ...
      rows = response
      if not rows or (limit is not None and len(rows) < min(limit, 200)):
        return rows, None
      return rows, page + 1
