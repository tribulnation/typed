...
class UniversalTransferHistoryEndpoint(RpcEndpoint):
  ...
  def universal_transfer_history_paged(
    ...
    *,
    ...
    Paged variant of `universal_transfer_history`: Requests `page` from 1 upwards and stops once it has covered the `totalCount` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[UniversalTransferRecord], int | None]:
      ...
      if not rows or (
        total is not None
        and (page - 1 + 1) * (min(limit, 500) if limit is not None else 500) >= total
      ):
        return rows, None
      return rows, page + 1
