...
class ClosedRiskUnits(RpcEndpoint):
  ...
  def closed_risk_units_paged(
    ...
    *,
    ...
    Paged variant of `closed_risk_units`: Requests `current` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(current: int) -> tuple[Sequence[ClosedRiskUnitRecord], int | None]:
