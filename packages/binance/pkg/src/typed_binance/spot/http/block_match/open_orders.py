...
class OpenOrders(RpcEndpoint):
  ...
  def open_orders_paged(
    ...
    *,
    ...
    Paged variant of `open_orders`: Requests `page` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page: int) -> tuple[Sequence[BlockMatchOpenOrder], int | None]:
      ...
      if not rows_ or (total is not None and (page - 1 + 1) * min(rows, 2000) >= total):
        return rows_, None
      return rows_, page + 1
