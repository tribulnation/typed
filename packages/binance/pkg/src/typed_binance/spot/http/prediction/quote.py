...
class PredictionQuote(TypedDict):
  ...
  feeDiscountBps: NotRequired[Decimal]
...
class Request(TypedDict):
  ...
  """Trade direction."""
  amountIn: str
  """Input amount in wei (18 decimals). Must be > 0. For `MARKET` orders, the minimum is approximately 1.5 USDT (varies by market depth); a smaller amount is rejected with `-9000 Your order amount is too small`. This minimum does not apply to `LIMIT` orders. Example: `1000000000000000000` = 1 USDT."""
  ...
  """Amount to auto-transfer into the prediction wallet before quoting, in wei. Must be > 0 if provided."""
  ...
  async def quote(
    ...
    *,
    ...
    if fee_rate_bps is not None:
      request['feeRateBps'] = fee_rate_bps
