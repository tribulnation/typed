# Generated by typed-dev — see CONTRIBUTING.md.
from decimal import Decimal
from typed_core import PaginatedResponse
from typing_extensions import Any, Literal, NotRequired, Sequence, TypedDict
from typed_binance.core import TimestampMillis
from typed_binance.core.endpoint.rpc import RpcEndpoint


class PredictionMarketTopicSummary(TypedDict):
  """Summary of a prediction market topic."""

  marketTopicId: int
  """Market topic ID."""
  vendor: str
  """Prediction liquidity vendor backing this topic."""
  chainId: str
  """Chain ID the topic settles on."""
  slug: str
  """URL-friendly topic slug."""
  title: str
  """Topic title."""
  question: str
  """Topic question shown to users."""
  description: NotRequired[str]
  """Topic description."""
  imageUrl: str
  """Topic display image URL."""
  topicType: str
  """Topic layout type."""
  chartType: NotRequired[str]
  """Chart rendering type for this topic."""
  symbol: NotRequired[str]
  """Underlying price symbol this topic tracks, when applicable."""
  participantCount: int
  """Number of participants in this topic."""
  collateral: str
  """Collateral asset symbol."""
  feeRateBps: int
  """Fee rate in basis points."""
  slippageBps: int
  """Default slippage tolerance in basis points."""
  isYieldBearing: bool
  """Whether the collateral backing this topic is yield-bearing."""
  tradeVolume: Decimal
  """Cumulative trade volume, in collateral units, as a decimal string."""
  liquidity: Decimal
  """Current liquidity, in collateral units, as a decimal string."""
  publishedAt: TimestampMillis
  """Time the topic was published."""
  startDate: TimestampMillis
  """Time the topic opens for trading."""
  endDate: TimestampMillis
  """Time the topic closes for trading."""
  status: str
  """Topic lifecycle status."""
  markets: list[dict[str, Any]]
  """Markets under this topic. Summarized here with no fields; see `spot.prediction.market_detail` for the full per-market shape."""


class Request(TypedDict):
  """Request parameters."""

  l1Category: NotRequired[str]
  """Level-1 category filter."""
  l2Category: NotRequired[str]
  """Level-2 category filter."""
  sortBy: NotRequired[
    Literal['RECOMMENDED', 'VOLUME', 'PARTICIPANTS', 'CREATED_TIME', 'END_DATE']
  ]
  """Sort field."""
  orderBy: NotRequired[Literal['ASC', 'DESC']]
  """Sort direction."""
  offset: NotRequired[int]
  """Pagination offset. Default `0`."""
  limit: NotRequired[int]
  """Page size. Default `20`, range 1-100."""


class PredictionMarketTopicList(TypedDict):
  """Paginated list of prediction market topics."""

  marketTopics: list[PredictionMarketTopicSummary]
  """Prediction market topics on this page."""
  total: int
  """Total number of market topics matching the filter."""
  offset: int
  """Pagination offset used for this page."""
  limit: int
  """Page size used for this page."""
  hasMore: bool
  """Whether further pages remain."""


class MarketList(RpcEndpoint):
  """Get a paginated list of prediction market topics, with optional category and sort filters."""

  def market_list_paged(
    self,
    *,
    l1category: str | None = None,
    l2category: str | None = None,
    sort_by: Literal[
      'RECOMMENDED', 'VOLUME', 'PARTICIPANTS', 'CREATED_TIME', 'END_DATE'
    ]
    | None = None,
    order_by: Literal['ASC', 'DESC'] | None = None,
    limit: int | None = None,
    validate: bool | None = None,
  ) -> PaginatedResponse[PredictionMarketTopicSummary, int]:
    """Get a paginated list of prediction market topics, with optional category and sort filters.

    Paged variant of `market_list`: Advances `offset` by `len(rows)` and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).

    Args:
      l1category: Level-1 category filter.
      l2category: Level-2 category filter.
      sort_by: Sort field.
      order_by: Sort direction.
      limit: Page size. Default `20`, range 1-100.
      validate: Override this call's response validation; falls back to the client-level default when omitted.

    References:
      - [Official docs](https://developers.binance.com/en/docs/catalog/web3-wallet-prediction-trading/api/rest-api/market-data#list-prediction-markets)
    """

    async def next(
      offset: int,
    ) -> tuple[Sequence[PredictionMarketTopicSummary], int | None]:
      response = await self.market_list(
        l1category=l1category,
        l2category=l2category,
        sort_by=sort_by,
        order_by=order_by,
        limit=limit,
        offset=offset,
        validate=validate,
      )
      rows = response.get('marketTopics')
      rows = rows if rows is not None else []
      total = response.get('total')
      total = int(total) if total is not None else None
      if not rows or (total is not None and (offset + len(rows)) >= total):
        return rows, None
      return rows, offset + len(rows)

    return PaginatedResponse(0, next)

  async def market_list(
    self,
    *,
    l1category: str | None = None,
    l2category: str | None = None,
    sort_by: Literal[
      'RECOMMENDED', 'VOLUME', 'PARTICIPANTS', 'CREATED_TIME', 'END_DATE'
    ]
    | None = None,
    order_by: Literal['ASC', 'DESC'] | None = None,
    offset: int | None = None,
    limit: int | None = None,
    validate: bool | None = None,
  ) -> PredictionMarketTopicList:
    """Get a paginated list of prediction market topics, with optional category and sort filters.

    Args:
      l1category: Level-1 category filter.
      l2category: Level-2 category filter.
      sort_by: Sort field.
      order_by: Sort direction.
      offset: Pagination offset. Default `0`.
      limit: Page size. Default `20`, range 1-100.
      validate: Override this call's response validation; falls back to the client-level default when omitted.

    References:
      - [Official docs](https://developers.binance.com/en/docs/catalog/web3-wallet-prediction-trading/api/rest-api/market-data#list-prediction-markets)
    """
    request: Request = Request()
    if l1category is not None:
      request['l1Category'] = l1category
    if l2category is not None:
      request['l2Category'] = l2category
    if sort_by is not None:
      request['sortBy'] = sort_by
    if order_by is not None:
      request['orderBy'] = order_by
    if offset is not None:
      request['offset'] = offset
    if limit is not None:
      request['limit'] = limit
    return await self.request(
      request,
      method='GET',
      path='/sapi/v1/w3w/wallet/prediction/market/list',
      meta={'security': 'SIGNED', 'signed': True},
      validate=validate,
      request_type=Request,
      response_type=PredictionMarketTopicList,
    )
