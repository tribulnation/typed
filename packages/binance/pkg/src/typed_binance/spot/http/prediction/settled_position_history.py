...
class PredictionSettledPosition(TypedDict):
  ...
  """Realized profit and loss, as a decimal string."""
  claimAmount: NotRequired[Decimal]
...
class SettledPositionHistory(RpcEndpoint):
  ...
  def settled_position_history_paged(
    self,
    *,
    wallet_address: str,
  ...
  ) -> PaginatedResponse[PredictionSettledPosition, int]:
    """Get the authenticated user's settled (resolved) prediction position history with optional filters.

    Paged variant of `settled_position_history`: Advances `offset` by `len(rows)` and stops once it has covered the `totalCount` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      offset: int,
    ) -> tuple[Sequence[PredictionSettledPosition], int | None]:
