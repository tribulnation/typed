...
class WbethRewardsHistory(RpcEndpoint):
  ...
  def wbeth_rewards_history_paged(
    self,
    *,
  ...
  ) -> PaginatedResponse[WbethReward, int]:
    """Query this account's WBETH staking rewards history.

    Paged variant of `wbeth_rewards_history`: Requests `current` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(current: int) -> tuple[Sequence[WbethReward], int | None]:
  ...
  async def wbeth_rewards_history(
    ...
    *,
    ...
    return await self.request(
      ...
      response_type=WbethRewardsHistoryPage,
    )
