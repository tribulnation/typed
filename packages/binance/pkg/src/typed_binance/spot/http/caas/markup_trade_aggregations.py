...
class MarkupTradeAggregation(TypedDict):
  ...
  """Fee group name."""
  totalVolumeUsdt: NotRequired[Decimal]
  ...
  markupMakerIncome: NotRequired[Decimal]
  """Maker markup income for the bucket, as a decimal string."""
  markupTakerIncome: NotRequired[Decimal]
...
class MarkupTradeAggregations(RpcEndpoint):
  ...
  def markup_trade_aggregations_paged(
    ...
    *,
    ...
    Paged variant of `markup_trade_aggregations`: Requests `pageIndex` from 1 upwards and stops once it has covered the `data.total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(
      page_index: int,
    ) -> tuple[Sequence[MarkupTradeAggregation], int | None]:
