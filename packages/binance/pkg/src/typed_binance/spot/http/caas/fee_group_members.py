...
class FeeGroupMembers(RpcEndpoint):
  ...
  def fee_group_members_paged(
    ...
    *,
    ...
    Paged variant of `fee_group_members`: Requests `pageIndex` from 1 upwards and stops once it has covered the `data.total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_index: int) -> tuple[Sequence[FeeGroupMember], int | None]:
