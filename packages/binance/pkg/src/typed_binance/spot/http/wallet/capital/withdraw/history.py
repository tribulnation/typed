...
class History(RpcEndpoint):
  ...
  def history_paged(
    ...
    *,
  ...
  ) -> PaginatedResponse[WithdrawRecord, int]:
    """Fetch withdraw history. If both `startTime` and `endTime` are sent, the window between them must be less than 90 days; if `withdrawOrderId` is sent, the window must be less than 7 days, and defaults to the last 7 days when `startTime`/`endTime` are also omitted. Otherwise defaults to the last 90 days.

    Paged variant of `history`: Advances `offset` by `len(rows)` and stops on the first page shorter than `limit`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(offset: int) -> tuple[Sequence[WithdrawRecord], int | None]:
      ...
      rows = response
      if not rows or len(rows) < (min(limit, 1000) if limit is not None else 1000):
        return rows, None
