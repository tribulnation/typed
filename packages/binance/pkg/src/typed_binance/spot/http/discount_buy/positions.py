...
class DiscountBuyPosition(TypedDict):
  ...
  """Purchase status."""
  knockoutApr: NotRequired[Decimal]
...
class Positions(RpcEndpoint):
  ...
  def positions_paged(
    ...
    *,
    ...
    Paged variant of `positions`: Requests `pageIndex` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_index: int) -> tuple[Sequence[DiscountBuyPosition], int | None]:
