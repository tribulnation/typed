...
class OrderHistory(RpcEndpoint):
  ...
  def order_history_paged(
    ...
    *,
    ...
    Paged variant of `order_history`: Requests `current` from 1 upwards and stops once it has covered the `total` items the response reports, or on an empty page. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(current: int) -> tuple[Sequence[EquityOrderHistoryRow], int | None]:
      ...
      if not rows or (
        total is not None
        and (current - 1 + 1) * (min(size, 100) if size is not None else 20) >= total
      ):
        return rows, None
