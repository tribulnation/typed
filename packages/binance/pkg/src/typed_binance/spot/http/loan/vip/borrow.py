...
class Request(TypedDict):
  ...
  loanAccountId: int
  """Loan receiving account ID. Must be under the same master account as `collateralAccountId`; only master account applications are supported."""
  loanCoin: str
  """Coin to borrow."""
  loanAmount: float
  """Amount to borrow."""
  ...
  collateralCoin: str
  """Collateral coin(s) securing the loan, comma-separated for multiple."""
  isFlexibleRate: bool
  """`true` for a flexible rate loan, `false` for a fixed rate loan."""
  loanTerm: NotRequired[int]
  """Loan term in days, e.g. 30 or 60. Mandatory when `isFlexibleRate` is `false`; optional for a flexible rate loan."""
  ...
  async def borrow(
    ...
    *,
    ...
    return await self.request(
      ...
      response_type=VipLoanBorrowOrder,
    )
