...
class ContinuousKline(StreamEndpoint):
  """Pushes kline/candlestick data for a specific contract type of a pair (perpetual, current-quarter, or next-quarter delivery), every 250ms."""
  ...
  def continuous_kline(
    ...
    *,
  ...
  ) -> StreamManager[CoinMContinuousKlineEvent, Any, Any]:
    """Pushes kline/candlestick data for a specific contract type of a pair (perpetual, current-quarter, or next-quarter delivery), every 250ms.

    Args:
      pair: Underlying pair, e.g. `BTCUSD`.
      contract_type: Contract type, templated into the channel in lowercase.
    ...
    return self.subscribe(
      ...
      validate=validate,
      response_type=CoinMContinuousKlineEvent,
    )
