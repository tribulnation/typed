# Generated by typed-dev — see CONTRIBUTING.md.
from typing_extensions import Literal, NotRequired, TypedDict
from typed_core.validation import validator
from typed_binance.core import TimestampMillis
from typed_binance.core.endpoint.rpc import RpcEndpoint


class PmNewUmOrder(TypedDict):
  """Place new UM order."""

  clientOrderId: NotRequired[str]
  """Client Order ID."""
  cumQty: NotRequired[str]
  """Cum Qty."""
  executedQty: NotRequired[str]
  """Executed Qty."""
  orderId: NotRequired[int]
  """Normal orderID after trigger if appliable, only have when the strategy is triggered."""
  origQty: NotRequired[str]
  """Orig Qty."""
  price: NotRequired[str]
  """Price."""
  reduceOnly: NotRequired[bool]
  """Reduce Only."""
  side: NotRequired[Literal['BUY', 'SELL']]
  """Side."""
  positionSide: NotRequired[Literal['BOTH', 'LONG', 'SHORT']]
  """BOTH means that it is the position of One-way Mode."""
  status: NotRequired[
    Literal[
      'NEW',
      'PARTIALLY_FILLED',
      'FILLED',
      'CANCELED',
      'REJECTED',
      'EXPIRED',
      'EXPIRED_IN_MATCH',
    ]
  ]
  """Order status."""
  symbol: NotRequired[str]
  """Trade symbol, if existing."""
  timeInForce: NotRequired[Literal['GTC', 'IOC', 'FOK', 'GTX', 'GTD']]
  """Time In Force."""
  type: NotRequired[Literal['LIMIT', 'MARKET']]
  """Normal order type after trigger if appliable."""
  selfTradePreventionMode: NotRequired[
    Literal['NONE', 'EXPIRE_TAKER', 'EXPIRE_BOTH', 'EXPIRE_MAKER']
  ]
  """self trading preventation mode."""
  goodTillDate: NotRequired[TimestampMillis]
  """order pre-set auot cancel time for TIF GTD order."""
  updateTime: NotRequired[TimestampMillis]
  """last update time."""
  priceMatch: NotRequired[
    Literal[
      'OPPONENT',
      'OPPONENT_5',
      'OPPONENT_10',
      'OPPONENT_20',
      'QUEUE',
      'QUEUE_5',
      'QUEUE_10',
      'QUEUE_20',
    ]
  ]
  """Price Match."""


class PmUmLimitOrder(TypedDict):
  """A UM limit order: filled at price or better. Requires timeInForce, quantity and price."""

  symbol: str
  """Symbol."""
  side: Literal['BUY', 'SELL']
  """Order side."""
  type: Literal['LIMIT']
  """Order type."""
  positionSide: NotRequired[Literal['BOTH', 'LONG', 'SHORT']]
  """Default `BOTH` for One-way Mode ; `LONG` or `SHORT` for Hedge Mode. It must be sent in Hedge Mode."""
  timeInForce: Literal['GTC', 'IOC', 'FOK', 'GTX', 'GTD']
  """Valid values."""
  quantity: float
  """Place amount."""
  reduceOnly: NotRequired[Literal['true', 'false']]
  """\"true" or "false". default "false". Cannot be sent in Hedge Mode ."""
  price: float
  """Order price."""
  newClientOrderId: NotRequired[str]
  """A unique id among open orders. Automatically generated if not sent. Can only be string following the rule: `^[.A-Z:/a-z0-9_-]{1,32}$`."""
  newOrderRespType: NotRequired[Literal['ACK', 'RESULT']]
  """`ACK`, `RESULT`, default `ACK`."""
  priceMatch: NotRequired[
    Literal[
      'OPPONENT',
      'OPPONENT_5',
      'OPPONENT_10',
      'OPPONENT_20',
      'QUEUE',
      'QUEUE_5',
      'QUEUE_10',
      'QUEUE_20',
    ]
  ]
  """only avaliable for `LIMIT`/`STOP`/`TAKE_PROFIT` order; can be set to `OPPONENT`/ `OPPONENT_5`/ `OPPONENT_10`/ `OPPONENT_20`: /`QUEUE`/ `QUEUE_5`/ `QUEUE_10`/ `QUEUE_20`; Can't be passed together with `price`."""
  selfTradePreventionMode: NotRequired[
    Literal['NONE', 'EXPIRE_TAKER', 'EXPIRE_BOTH', 'EXPIRE_MAKER']
  ]
  """`NONE`:No STP / `EXPIRE_TAKER`:expire taker order when STP triggers/ `EXPIRE_MAKER`:expire taker order when STP triggers/ `EXPIRE_BOTH`:expire both orders when STP triggers."""
  goodTillDate: NotRequired[TimestampMillis]
  """order cancel time for timeInForce `GTD`, mandatory when `timeInforce` set to `GTD`; order the timestamp only retains second-level precision, ms part will be ignored; The goodTillDate timestamp must be greater than the current time plus 600 seconds and smaller than 253402300799000Mode. It must be sent in Hedge Mode."""
  recvWindow: NotRequired[int]
  """Number of milliseconds after `timestamp` the request remains valid for."""


class PmUmMarketOrder(TypedDict):
  """A UM market order: filled immediately at the best available price. Requires quantity."""

  symbol: str
  """Symbol."""
  side: Literal['BUY', 'SELL']
  """Order side."""
  type: Literal['MARKET']
  """Order type."""
  positionSide: NotRequired[Literal['BOTH', 'LONG', 'SHORT']]
  """Default `BOTH` for One-way Mode ; `LONG` or `SHORT` for Hedge Mode. It must be sent in Hedge Mode."""
  quantity: float
  """Place amount."""
  reduceOnly: NotRequired[Literal['true', 'false']]
  """\"true" or "false". default "false". Cannot be sent in Hedge Mode ."""
  newClientOrderId: NotRequired[str]
  """A unique id among open orders. Automatically generated if not sent. Can only be string following the rule: `^[.A-Z:/a-z0-9_-]{1,32}$`."""
  newOrderRespType: NotRequired[Literal['ACK', 'RESULT']]
  """`ACK`, `RESULT`, default `ACK`."""
  selfTradePreventionMode: NotRequired[
    Literal['NONE', 'EXPIRE_TAKER', 'EXPIRE_BOTH', 'EXPIRE_MAKER']
  ]
  """`NONE`:No STP / `EXPIRE_TAKER`:expire taker order when STP triggers/ `EXPIRE_MAKER`:expire taker order when STP triggers/ `EXPIRE_BOTH`:expire both orders when STP triggers."""
  recvWindow: NotRequired[int]
  """Number of milliseconds after `timestamp` the request remains valid for."""


class NewUmOrder(RpcEndpoint):
  """New UM Order"""

  async def new_um_order(
    self,
    pm_um_limit_order_pm_um_market_order: PmUmLimitOrder | PmUmMarketOrder,
    *,
    validate: bool | None = None,
  ) -> PmNewUmOrder:
    """Place new UM order.

    Args:
      pm_um_limit_order_pm_um_market_order: New UM order request. Shape depends on order type; see docs/spec/authoring.md rule 0 -- a discriminated-union payload whose required fields differ by variant.

    References:
      - [Official docs](https://developers.binance.com/en/docs/catalog/advanced-trading-derivatives-trading-portfolio-margin/api/rest-api/trade#new-um-order)
    """
    params = dict(pm_um_limit_order_pm_um_market_order)
    _Response = PmNewUmOrder
    _validator = validator[_Response](_Response)
    return await self.authed_request(
      'POST',
      '/papi/v1/um/order',
      params=params,
      validator=_validator,
      validate=validate,
    )
