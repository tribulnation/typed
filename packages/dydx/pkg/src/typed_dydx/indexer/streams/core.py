...
"""A stream's subscribe-reply type -- `Any` for a call that passes no `reply_type`, so a
channel whose spec declares no `reply` keeps the untyped slot it always had."""
...
class StreamsMixin:
  ...
  def subscribe(
    ...
    *,
    ...
    reply_type: type[R] | UnionType | None = None,
  ) -> StreamManager[T, R, Unsubscribed]:
    ...
    validated through `response_type`'s validator, and the subscribe reply's own
    `contents`, validated through `reply_type`'s (ADR 0022) and handed back as
    `Stream.reply`. The two are separate types on purpose: `response_type` describes a
    pushed message, and the reply is a different shape on several channels
    (`v4_orderbook` acks with `{price, size}` objects and pushes `[price, size]` tuples).
    ...
    Args:
      ...
      validate: Per-call override of reply and pushed-payload validation.
      request_type: The generated parameters type, used to serialize `request`.
      response_type: The generated payload type, used to validate each pushed message's
        extracted `contents`; never applied to the subscribe reply.
      reply_type: The generated reply type, used to validate the subscribe reply's
        extracted `contents`; never applied to a pushed message. Omitted for a channel
        whose spec declares no `reply`, whose reply is then handed back as-is.
    """
    values: dict[str, Any] = (
    ...
    async def impl() -> Stream[T, R, Unsubscribed]:
      ...
      # The `subscribed` reply's `contents` goes through `reply_type`, never
      # `response_type`: that type describes a *pushed* message, and on several channels
      # the reply is a different shape altogether -- `v4_orderbook` replies with the full
      # book as `{price, size}` objects while its pushes carry `[price, size]` tuples
      # (also `v4_block_height`, `v4_candles`, `v4_trades`), so validating the reply with
      # the push type raised `ValidationError` before the first message ever arrived (SDK
      # PoC report, 2026-09-06, live on testnet). Each channel's spec now declares its own
      # `reply` schema, generated as the `reply_type=` passed here (ADR 0022).
      try:
        reply_contents = subscription.reply['contents']
        reply: R = (
          validator(cast(type, reply_type)).python(reply_contents)
          if should_validate and reply_type is not None
          else reply_contents
        )
      except Exception:
        # No Stream has been returned to own cleanup if the acknowledgement fails.
