...
class List(RpcEndpoint):
  ...
  def list_paged(
    ...
    *,
    ...
    Paged variant of `list`: Passes each page's token back as `cursor` and stops when a response carries no `pagination.next_cursor`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(cursor: str) -> tuple[Sequence[Product], str | None]:
