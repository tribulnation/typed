...
class RpcClient(Protocol):
  ...
  async def authed_raw_request(
    ...
    *,
    ...
    `validate` is accepted, not for response validation (there is no schema to validate
    a raw binary file against -- see `RetrieveExport`'s own generated method), but so
    every `rpc`-kind endpoint carries the same `validate` parameter uniformly
    (`docs/production_standards.md` S8). It has no effect.
...
class RpcEndpoint:
  ...
  async def authed_raw_request(
    ...
    *,
    ...
    return await self.client.authed_raw_request(path, data, validate=validate)
