...
`Kraken.new(...)` builds all three top-level surfaces -- `spot` (REST), `streams`
(WebSocket subscriptions), and `trading_ws` (WebSocket trading RPCs) -- and its
`__aenter__` enters them concurrently (`asyncio.gather`), so `async with Kraken.new(...)`
is the only thing you need: every sub-surface underneath (`spot.account`, `spot.trading`,
`streams.market_data`, `trading_ws.add_order`, ...) bottoms out lazily on first use, the
same as direct construction. You never enter a sub-client separately.
...
`trading_ws` is a separate top-level surface, not part of `streams`: its methods
(`add_order`, `cancel_order`, `edit_order`, ...) are plain WebSocket RPC calls, not
subscriptions -- `await` them directly, the same as an HTTP call:
...
`Kraken.new()` bundles three independent top-level surfaces:
...
- `streams` -- WebSocket v2 channel subscriptions, over two separate connections.
  `market_client` connects to the public `wss://ws.kraken.com/v2` and backs
  `streams.market_data`. `private_client` connects to the authenticated
  `wss://ws-auth.kraken.com/v2` and backs `streams.private` (account channel
  subscriptions).
- `trading_ws` -- WebSocket v2 trading RPCs (order placement/cancellation), request/reply
  rather than subscriptions. Reaches Kraken over the *same* `private_client` connection
  `streams.private` uses -- Kraken serves both over one authenticated socket -- but is
  its own top-level surface, kept out of `streams` since its calls are `await`ed
...
client = Kraken.new(public=True, http=http)
```
