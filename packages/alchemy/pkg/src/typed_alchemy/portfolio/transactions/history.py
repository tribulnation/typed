...
class History(PortfolioEndpoint):
  ...
  def history_paged(
    ...
    *,
    ...
    Paged variant of `history`: Passes each page's token back as `after` and stops when a response carries no `after`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(after: str) -> tuple[Sequence[Transaction], str | None]:
