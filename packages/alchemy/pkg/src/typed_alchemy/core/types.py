...
NFT metadata timestamps (`timeLastUpdated`, `lastIngestedAt`) are epoch milliseconds (an
NFT mint's own `timestamp` has only ever been observed `null`, so its shape is undeclared); Prices API timestamps (`lastUpdatedAt`) are RFC 3339
