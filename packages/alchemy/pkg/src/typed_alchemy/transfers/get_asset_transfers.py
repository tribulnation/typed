...
class GetAssetTransfers(ChainRpc):
  ...
  def get_asset_transfers_paged(
    ...
    *,
    ...
    Paged variant of `get_asset_transfers`: Passes each page's token back as `pageKey` and stops when a response carries no `pageKey`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_key: str) -> tuple[Sequence[Transfer], str | None]:
