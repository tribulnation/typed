...
class GetNftSales(ChainNft):
  ...
  def get_nft_sales_paged(
    ...
    *,
    ...
    Paged variant of `get_nft_sales`: Passes each page's token back as `pageKey` and stops when a response carries no `pageKey`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_key: str) -> tuple[Sequence[NftSale], str | None]:
