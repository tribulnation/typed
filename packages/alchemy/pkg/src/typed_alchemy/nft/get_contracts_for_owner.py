...
class GetContractsForOwner(ChainNft):
  ...
  def get_contracts_for_owner_paged(
    ...
    *,
    ...
    Paged variant of `get_contracts_for_owner`: Passes each page's token back as `pageKey` and stops when a response carries no `pageKey`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_key: str) -> tuple[Sequence[OwnedContract], str | None]:
