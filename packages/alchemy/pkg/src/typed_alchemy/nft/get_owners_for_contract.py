...
class GetOwnersForContract(ChainNft):
  ...
  def get_owners_for_contract_paged(
    ...
    *,
    ...
    Paged variant of `get_owners_for_contract`: Passes each page's token back as `pageKey` and stops when a response carries no `pageKey`. Awaitable (flattens every page) or async-iterable (one page at a time).
    ...
    async def next(page_key: str) -> tuple[Sequence[NftContractOwner], str | None]:
