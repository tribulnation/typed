...
Endpoints that page results expose a `_paged` sibling. Each one is
`PaginatedResponse`-shaped: **both** awaitable and async-iterable, never just one or the
other.
...
async with Alchemy.new() as client:
  transfers = await client.transfers(network='ethereum').get_asset_transfers_paged(
    from_block='0x0',
...
async with Alchemy.new() as client:
  pages = client.portfolio.tokens_paged(
    addresses=[
      {
...
async with Alchemy.new() as client:
  owned_nfts = await client.nft(network='ethereum').get_nfts_for_owner_paged(
    owner='vitalik.eth',
    page_size=10,
  )
  print(len(owned_nfts))

  balance_pages = client.token(network='ethereum').get_token_balances_paged(
    address='0x1E6E8695FAb3Eb382534915eA8d7Cc1D1994B152',
    token_spec='erc20',
    max_count=10,
  )
